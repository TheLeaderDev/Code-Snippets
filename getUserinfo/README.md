...
<p align="center">Very important: This function gets some of its info from the Check Host API, so it definitely needs an internet connection. Keep in mind that if the site's policies change in the future, the function might run into issues since it's dependent on it. But don’t worry—if that</p>
...
<p align="center"> خیلی مهم: این تابع بعضی اطلاعاتش رو از API سایت Check Host می‌گیره، پس حتماً اینترنت لازم داره. یادت باشه که اگه سیاست‌های سایت در آینده عوض بشه، ممکنه تابع هم به مشکل بخوره چون وابسته‌س. البته اگه این اتفاق بیفته، من هم سعی می‌کنم تابع رو آپدیت کنم.</p>
